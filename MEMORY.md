...
## Learnings (latest)
- 2026-03-04 (resolution): LinkedIn PA agent cron stuck with stale `runningAtMs` flag causing missed runs. Recovery: manually executed agent script, then reset cron state via disable/enable cycle, and removed duplicate cron job entry. However, the job remained enabled in OpenClaw after recovery, causing continued hourly runs despite CRON_JOBS.md documenting it as disabled. Discovered via cron execution notification (03:04 UTC); disabled job via `openclaw cron disable 79c865ab-9181-4acc-8065-15f8504fe68a`. Configuration drift corrected. Lesson: After manual cron recovery, verify both the job's functional state and its documented status; enable/disable operations should be confirmed. Cron state may become stale; a simple disable/enable clears `runningAtMs` and resets schedule. Consider investigating root cause (race condition on completion signal).
