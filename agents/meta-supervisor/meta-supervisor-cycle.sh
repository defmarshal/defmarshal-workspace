...
def check_agent(job):
    ...
    if name == "agni-cron":
        # Check for plan file after last run
        today_str = datetime.datetime.now().strftime("%Y-%m-%d")
        today_plans = [p for p in glob.glob(f"{workspace}/agents/agni/plans/plan-{today_str}_*.md") if os.path.getmtime(p) > last_run_sec]
        if not today_plans:
            findings.append("No new plan file after last run")
            outcome = "no_output"
        else:
            expected_report = f"{workspace}/agents/rudra/reports/report-{os.path.basename(today_plans[0]).replace('plan-','').replace('.md','')}.md"
            expected_log = f"{workspace}/agents/rudra/logs/exec-{os.path.basename(today_plans[0]).replace('plan-','').replace('.md','')}.log"
        ...
        if not os.path.exists(digest_file):
            findings.append("Daily digest not found")
            outcome = "no_output"
        else:
            # Check if file modified after last run
            if os.path.getmtime(digest_file) < last_run_sec:
                findings.append("Daily digest not updated")
                outcome = "no_output"
