...
cleanup_idea_branches() {
  ...
  while IFS= read -r branch; do
    ...
    if git merge-base --quiet HEAD "refs/heads/$branch_name" 2>/dev/null; then
  ...
  if (( none_found )); then
    ...
    log "Idea branch cleanup: deleted=$cleaned, skipped_recent=$skipped_recent, skipped_unmerged=$skipped_merged"
  fi
}
